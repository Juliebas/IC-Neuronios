...
def Main(S, g, val, plot = True, t_up_plot = 50000):
    '''
    ...
    val : list | valores nos quais os vetores são inibitórios
    ...
    while s != 0:
        ...
        if plot and n%t_up_plot == 0:
            ...
            plt.subplot(2, 1, 1)
            plt.plot(neu, n_ativos, color = 'g', label = "Soma de Neurônios ativos")
            ...
            plt.subplot(2, 1, 2)
            posicoes = {}
            colunas, linhas = descobrir_dimensoes(S)
            for m in S.nodes():
                x = m % colunas    # O resto da divisão dá a coluna
                y = m // colunas   # A divisão inteira dá a linha
                posicoes[m] = (x, -y) # Invertemos o y para desenhar de cima para baixo
            nx.draw(S, pos = posicoes, node_color= [node_colors[i] for i in nodes_s], nodelist = nodes_s, node_shape = "s")
            nx.draw(S, pos = posicoes, node_color= [node_colors[i] for i in nodes_o], nodelist = nodes_o, node_shape = "o")
            plt.pause(0.01)
        n += 1
...
def descobrir_dimensoes(G):
    N = len(G.nodes)
    graus = dict(G.degree())
    
    # 1. Se o grau máximo for 2 (ou menor), é uma grid de 1 linha (1xN)
    max_grau = max(graus.values())
    if max_grau <= 2:
        return 1, N
        
    # 2. Conta quantos nós estão nas bordas (grau 3)
    nos_grau_3 = sum(1 for n, grau in graus.items() if grau == 3)
    
    # 3. Calcula a soma esperada de Linhas + Colunas usando a fórmula
    soma_L_C = (nos_grau_3 / 2) + 4
    
    # 4. Encontra os dois números que multiplicados dão N e somados dão soma_L_C
    for L in range(1, N + 1):
        if N % L == 0:  # Se L é um divisor de N
            C = N // L
            if L + C == soma_L_C:
                return L, C # Retorna o par (Linhas, Colunas)
                
    return None, None # Retorna None se não for uma grid perfeita